...
describe TournamentMailer do
  # describe 'user_joined_tournament' do
    # let(:owner) { FactoryGirl.create(:user, role: UserSoldier, name: 'Lucas', email: 'lucas@email.com') }
    ...
    # let(:user) { mock_model(User, name: 'User', email: 'user@email.com', clan_id: clan.id) }
...
#     
    ...
    # it 'assigns owner @name' do
      # mail.body.encoded.should match(owner.name)
    # end
#     
...
#   
  # describe 'tournament_min_team_reached' do
    # let!(:owner) { FactoryGirl.create(:user, role: UserSoldier, name: 'Lucas', email: 'lucas@email.com') }
    # let!(:tanker1) { FactoryGirl.create(:user, name: 'Tanker1') }
    # let!(:tanker2) { FactoryGirl.create(:user, name: 'Tanker2') }
...
#   
  # describe 'user_left_tournament' do
    # let(:owner) { FactoryGirl.create(:user, role: UserSoldier, name: 'Lucas', email: 'lucas@email.com') }
    # let!(:tournament) { FactoryGirl.create(:tournament, user: owner, name: 'My Tournament', team_name: "Team Name", wot_team_link: 'http://www.somewhere.com/team') }
    # let(:user) { mock_model(User, name: 'User', email: 'user@email.com') }
